...
ss <- table(daily_cases$state_fips) # entries per state
cc <- table(daily_cases$county_fips) # entries per county
sum(cc<=10)  # currently 335 counties with fewer than 5 days of entries

# drop counties have fewer than 10 days of data 
data<-daily_cases[daily_cases$county_fips %in% names(cc)[cc>10],]
cc2<-table(data$county_fips)
...
res2 <- glmer(new_cases~zday+zday2
              +(zday+zday2|county_fips)+offset(logpop), data=data, 
              family=poisson(link="log"))
...
res3 <- glmer(formula(ff3), data=data, 
              family=poisson(link="log"))
